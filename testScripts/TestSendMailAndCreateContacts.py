        ...
        for id, data in enumerate(isExecuteColumn[1:]):
            ...
            if data.value.lower() == "y":
                ...
                if useFrameWorkName == "数据":
                    logging.info("******调用数据驱动******")
                    # 获取测试用例表中，执行框架为数据驱动的用例所使用的数据sheet名
                    ...
                    if result:
                        ...
                        successfulCase +=1
                        WriteTestResult.writeTestResult(caseSheet,rowNo=id+2,colsNo="testCase",testResult="pass")
                    else:
                        logging.info("用例'%s' 执行失败" % caseName)
                        WriteTestResult.writeTestResult(caseSheet,rowNo=id+2,colsNo="testCase",testResult="faild")
                    ...
                    for index in range(2,stepNums+1):
                        ...
                        try:
                            ...
                            WriteTestResult.writeTestResult(caseStepObj,rowNo=index,colsNo="testStep",testResult="faild"
                                                            ,errorInfo=str(errorInfo),picPath=capturePic)
                        else:
                            successfulSteps+=1
                            logging.info("执行步骤'%s'成功" % stepRow[testStep_testStepDescribe-1].value)
                            WriteTestResult.writeTestResult(caseStepObj,rowNo=index,colsNo="testStep",testResult="pass")
                ...
                # 清空不需要执行用例的执行时间和执行结果
                WriteTestResult.writeTestResult(caseSheet,rowNo=id+2,colsNo="testCase",testResult="")
